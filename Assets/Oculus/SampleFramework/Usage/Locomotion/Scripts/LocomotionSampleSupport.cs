...
public class LocomotionSampleSupport : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        DebugUIBuilder.instance.AddLabel("Locomotion Options:");
        DebugUIBuilder.instance.AddButton("Teleport", SetupRightTurnLeftTeleport);
        DebugUIBuilder.instance.AddButton("Slide", SetupWalkOnly);
        DebugUIBuilder.instance.AddDivider();
        DebugUIBuilder.instance.AddButton("Quit Game", QuitGame);
        ...
        if (EscapeGameManager.Instance.teleportLocomotion)
        {
            SetupRightTurnLeftTeleport();
        }
        else
        {
            SetupWalkOnly();
    ...
    void InitiateQuit()
    {
        ...
        toRemove.Add(DebugUIBuilder.instance.AddLabel("Are you sure that you want to quit?").gameObject);
        toRemove.Add(DebugUIBuilder.instance.AddButton("Yes", QuitGame).gameObject);
        toRemove.Add(DebugUIBuilder.instance.AddButton("No", CancelQuit).gameObject);
        StartCoroutine(TimedCancelQuit());
    }

    IEnumerator TimedCancelQuit()
    {
    ...
    {
        if (!quitCancelled)
            StopCoroutine(TimedCancelQuit());
        quitCancelled = false;
        
        while(toRemove.Count > 0)
        {
            GameObject temp = toRemove[0];
            toRemove.RemoveAt(0);
            Destroy(temp);
        }
    }
    ...
    void QuitGame()
    {
        StopCoroutine(TimedCancelQuit());
        quitCancelled = false;
        EscapeGameManager.Instance.QuitGame(true);
    }
    ...
    {
        EscapeGameManager.Instance.teleportLocomotion = true;
        SetupTeleportDefaults();
	...
	*/
    ...
    void SetupWalkOnly()
    {
        EscapeGameManager.Instance.teleportLocomotion = false;
        SetupTeleportDefaults();
